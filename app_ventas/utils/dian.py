...
def extraer_totales_por_tributo(factura):
    ...
    resumen = (
        factura.detallefactura_set
        .values('producto__impuesto__codigo_tributario')
        .annotate(total_tributo=Sum('monto_impuesto'))
    )
    
    mapa_impuestos = {
        item['producto__impuesto__codigo_tributario']: item['total_tributo'] 
        for item in resumen
    }
...
def calcular_cufe_cude(
    ...
    :param nit_emisor: NIT de la empresa emisora sin dígito de verificación (si es None, se toma de settings.NIT_EMISOR).
